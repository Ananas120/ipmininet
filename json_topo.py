...
from ipmininet.router.config import BGP, OSPF, OSPF6, RouterConfig, AF_INET6, set_rr, bgp_fullmesh, ebgp_session, SHARE, Rule
...
class JSONTopo(IPTopo):
    ...
    def add_daemons(self, router, daemons, default_daemons = []):
        ...
        # TODO handle IPTABLES daemons
        """ 
            elif d == '' :
                router.addDaemon(IPTables, rules=RULE)
        """
    ...
    def build(self, *args, **kwargs):
        ...
        ip_rule = [Rule('-A INPUT -m ttl --ttl-lt 255 -j DROP')]
        # command to add the rule :
        # ROUTER.addDaemon(IPTables, rules=ip_rule)
